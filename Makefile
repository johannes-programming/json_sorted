...
test:
	conda run -n base python make/env.py test_normdata --python=3.11 --recreate >/dev/null;
	conda run -n test_normdata pip install -e . >/dev/null;
	conda run -n test_normdata python run_tests.py;
	conda run -n test_normdata pip install mypy >/dev/null;
	conda run -n test_normdata python -m mypy --strict .;
	conda run -n test_normdata python -m mypy --strict -p normdata;
